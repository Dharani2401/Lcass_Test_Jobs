	...
	//allure report
			...
			resultsDir:  'natural\\allure_reports'
			}));
	...
	//Getting screenshots  
		...
		fs.emptyDir(`natural\\screenshots`, function (err) {
				 console.log(err);
			 });
			 jasmine.getEnv().addReporter({
				 specDone: function(result) {
					 if (result.status == 'failed') {
                         browser.takeScreenshot().then(function (png) {
                        var stream = fs.createWriteStream(`natural\\screenshots\\`+ result.fullName+ '.png');
		 ...
		 onComplete: function() {
			 //Getting HTML reportvar browserName, browserVersion;
				...
				testConfig = {
					reportTitle: `Test Report for testRun-natural`,
					outputPath: 'natural',
					outputFilename: 'ProtractorTestReport',
					//screenshotPath: `natural\\screenshots`,
					testBrowser: browserName,
					modifiedSuiteName: false,
					//screenshotsOnlyOnFailure: true,
					testPlatform: platform
				};
				new HTMLReport().from(`natural\\guitest-xmloutput.xml`, testConfig);	
		},
