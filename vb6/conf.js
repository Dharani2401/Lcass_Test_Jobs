	...
	//Getting screenshots  
		...
		fs.emptyDir(`vb6\\screenshots`, function (err) {
				 console.log(err);
			 });
			 jasmine.getEnv().addReporter({
				 specDone: function(result) {
					 if (result.status == 'failed') {
                         browser.takeScreenshot().then(function (png) {
                        var stream = fs.createWriteStream(`vb6\\screenshots\\`+ result.fullName+ '.png');
		 ...
		 onComplete: function() {
			 //Getting HTML reportvar browserName, browserVersion;
				...
				testConfig = {
					reportTitle: `Test Report for testRun-vb6`,
					outputPath: 'vb6',
					outputFilename: 'ProtractorTestReport',
					//screenshotPath: `vb6\\screenshots`,
					testBrowser: browserName,
					modifiedSuiteName: false,
					//screenshotsOnlyOnFailure: true,
					testPlatform: platform
				};
				new HTMLReport().from(`vb6\\guitest-xmloutput.xml`, testConfig);	
		},
