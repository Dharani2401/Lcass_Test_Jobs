	...
	//allure report
			...
			resultsDir:  'plsql\\allure_reports'
			}));
	...
	//Getting screenshots  
		...
		fs.emptyDir(`plsql\\screenshots`, function (err) {
				 console.log(err);
			 });
			 jasmine.getEnv().addReporter({
				 specDone: function(result) {
					 if (result.status == 'failed') {
                         browser.takeScreenshot().then(function (png) {
                        var stream = fs.createWriteStream(`plsql\\screenshots\\`+ result.fullName+ '.png');
		 ...
		 onComplete: function() {
			 //Getting HTML reportvar browserName, browserVersion;
				...
				testConfig = {
					reportTitle: `Test Report for testRun-plsql`,
					outputPath: 'plsql',
					outputFilename: 'ProtractorTestReport',
					//screenshotPath: `plsql\\screenshots`,
					testBrowser: browserName,
					modifiedSuiteName: false,
					//screenshotsOnlyOnFailure: true,
					testPlatform: platform
				};
				new HTMLReport().from(`plsql\\guitest-xmloutput.xml`, testConfig);	
		},
