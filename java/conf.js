	...
	//allure report
			...
			resultsDir:  'java\\allure_reports'
			}));
	...
	//Getting screenshots  
		...
		fs.emptyDir(`java\\screenshots`, function (err) {
				 console.log(err);
			 });
			 jasmine.getEnv().addReporter({
				 specDone: function(result) {
					 if (result.status == 'failed') {
                         browser.takeScreenshot().then(function (png) {
                        var stream = fs.createWriteStream(`java\\screenshots\\`+ result.fullName+ '.png');
		 ...
		 onComplete: function() {
			 //Getting HTML reportvar browserName, browserVersion;
				...
				testConfig = {
					reportTitle: `Test Report for testRun-java`,
					outputPath: 'java',
					outputFilename: 'ProtractorTestReport',
					//screenshotPath: `java\\screenshots`,
					testBrowser: browserName,
					modifiedSuiteName: false,
					//screenshotsOnlyOnFailure: true,
					testPlatform: platform
				};
				new HTMLReport().from(`java\\guitest-xmloutput.xml`, testConfig);	
		},
