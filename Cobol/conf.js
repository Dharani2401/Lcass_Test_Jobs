	...
	//allure report
			...
			resultsDir:  'Cobol\\allure_reports'
			}));
	...
	//Getting screenshots  
		...
		fs.emptyDir(`Cobol\\screenshots`, function (err) {
				 console.log(err);
			 });
			 jasmine.getEnv().addReporter({
				 specDone: function(result) {
					 if (result.status == 'failed') {
                         browser.takeScreenshot().then(function (png) {
                        var stream = fs.createWriteStream(`Cobol\\screenshots\\`+ result.fullName+ '.png');
		 ...
		 onComplete: function() {
			 //Getting HTML reportvar browserName, browserVersion;
				...
				testConfig = {
					reportTitle: `Test Report for testRun-Cobol`,
					outputPath: 'Cobol',
					outputFilename: 'ProtractorTestReport',
					//screenshotPath: `Cobol\\screenshots`,
					testBrowser: browserName,
					modifiedSuiteName: false,
					//screenshotsOnlyOnFailure: true,
					testPlatform: platform
				};
				new HTMLReport().from(`Cobol\\guitest-xmloutput.xml`, testConfig);	
		},
